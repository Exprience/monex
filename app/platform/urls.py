...
urlpatterns = [
	...
	url(r'^currency/sell/(?P<cid>[0-9]+)/(?P<pid>[0-9]+)/(?P<sid>[0-9]+)/$', v.CurrencySell.as_view(), name = 'currency_sell'),
	...
	url(r'^order/(?P<pk>[0-9]+)/$', v.Order.as_view(), name='order'),

	url(r'^chart/(?P<pk>[0-9]+)/$', v.Chart.as_view(), name='chart'),
