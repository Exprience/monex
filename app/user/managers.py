...
class UserDataManager(BaseDataManager):
    ...
    @staticmethod
    def get_instance():
        if UserDataManager.__instance is None:
            UserDataManager.__instance = UserDataManager()
        return UserDataManager.__instance

    @staticmethod
    def check_unique_user(status, value):
        client = UserDataManager.get_instance().setup_client('/MX_Check_Duplicate_Username_Email_HTTPService/MX_Check_Duplicate_Username_Email_HTTPPort?wsdl')
        result = client.service.MX_Check_Duplicate_Username_Email_HTTPOperation(status, value, False)
        return result

    @staticmethod
    def login(username, password):
        client = UserDataManager.get_instance().setup_client('/MX_Check_User_LoginService/MX_Check_User_LoginPort?wsdl')
        result = client.service.MX_Check_User_LoginOperation(username, hashlib.md5(password).hexdigest(), config.NOW)
        if result.isHavePrivilege:
            user = User()
    ...
    def register(username, email, password):
        client = UserDataManager.get_instance().setup_client('/MX_User_RegistrationService/MX_User_RegistrationPort?wsdl')
